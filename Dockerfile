...
RUN sed -i 's|mysqldump --defaults-file=/dev/fd/3 %s --single-transaction --quick --no-autocommit %s|mysqldump --defaults-file=/dev/fd/3 %s --single-transaction --quick --no-autocommit --column-statistics=0 %s|g' mysql.php
...
ENV AEGIR_HOSTNAME devaegir.local
